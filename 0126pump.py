...
# --- 펌프2: 연속 순환 설정 ---
PUMP2_FLOW_RATE = 0.13   # 펌프2 유속 (ml/min) - Masterflex 대용
...
def main_process():
    ...
    print("\n" + "="*50)
    print("       CNT 분리 자동화 시스템")
    print("="*50)
    print("\n[현재 설정]")
    print(f"  펌프2 유속: {PUMP2_FLOW_RATE} ml/min")
    ...
    # 펌프 연결
    p1 = EMPPump(PUMP1_PORT, PUMP1_ID)  # 주입용
    p2 = EMPPump(PUMP2_PORT, PUMP2_ID)  # 순환용
    
    print("\n--- 🚀 공정 시작 ---")
    
    # 펌프2 연속 운전 시작 (설정값 사용)
    p2.start_continuous(PUMP2_FLOW_RATE)
    ...
    try:
        while True:
            ...
            # [Step 2] 펌프1: 샘플 주입 (설정값 사용)
            p1.inject_volume(SAMPLE_VOLUME, SAMPLE_FLOW_RATE)
            ...
            # [Step 5] 펌프1: 1차 일루션 주입
            p1.inject_volume(ELUTION1_VOLUME, ELUTION1_FLOW_RATE)
            ...
            # [Step 8] 펌프1: 2차 일루션 주입
            p1.inject_volume(ELUTION2_VOLUME, ELUTION2_FLOW_RATE)
...
def test_pump():
    ...
    pump = EMPPump(PUMP1_PORT, PUMP1_ID)
